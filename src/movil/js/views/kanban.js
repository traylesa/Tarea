...
var VistaKanban = {
  ...
  renderizar: function(contenedor) {
    ...
    html += '<label style="display:flex;align-items:center;gap:4px;font-size:13px;margin-left:auto">';
    html += '<input type="checkbox" id="chk-m-documentado"' + (this._mostrarDocumentado ? ' checked' : '') + '> Doc';
    html += '</label>';
    html += '</div>';
  ...
  _inicializarEventos: function() {
    ...
    // Click en tarjeta -> BottomSheet detalle
  ...
  _abrirDetalleMovil: function(tarjeta) {
    ...
    info.innerHTML =
      '<div style="font-family:monospace;font-size:20px;font-weight:bold;margin-bottom:12px">' + codCar + '</div>' +
      '<div style="font-size:13px;color:#666;margin-bottom:4px">Transportista: <strong>' + transp + '</strong></div>' +
      '<div style="font-size:13px;color:#666;margin-bottom:12px">Estado: <strong>' + estado + '</strong> | Fase: <strong>' + fase + '</strong></div>';
    contenido.appendChild(info);

    if (typeof BottomSheet !== 'undefined') {
      BottomSheet.abrir({
        titulo: 'Detalle carga',
        contenido: contenido,
        opciones: [
  ...
  _aplicarFiltros: function(regs) {
    ...
    if (filtro === 'enproceso') {
      return resultado.filter(function(r) { return ['11', '12', '21', '22'].indexOf(r.fase) !== -1; });
    }
